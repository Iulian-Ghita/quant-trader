...
const ALPACA_KEY = "PKOLRB45CBIBZBI3RO6K5YH7NG";
const ALPACA_SECRET = "J1zwmBPmd5Us6zqJnRDhpch3Wd35GZD61ZLhYey364Zo";
...
async function fetchLivePrices() {
  try {
    ...
    const res = await fetch(`${BASE_URL}/stocks/trades/latest?symbols=${symbols}&feed=iex`, {
      headers: {
    ...
    SYMBOLS.forEach(sym => {
      if (data.trades && data.trades[sym]) {
        prices[sym] = data.trades[sym].p;
      } else {
        prices[sym] = fallbackPrices[sym];
      }
    });
    return prices;
  } catch (e) {
...
export default function TradingSystem() {
  ...
  const [priceHistory, setPriceHistory] = useState(() => { const h = {}; SYMBOLS.forEach(s => { h[s] = [fallbackPrices[s]]; }); return h; });
  const [signals, setSignals] = useState({});
  const [running, setRunning] = useState(false);
  const [tick, setTick] = useState(0);
  const [log, setLog] = useState([]);
  const [activeTab, setActiveTab] = useState("market");
  const [dataSource, setDataSource] = useState("simulat");
  ...
  const runTick = useCallback(async () => {
    ...
    const isLive = newPrices !== fallbackPrices;
    setDataSource(isLive ? "live" : "simulat");
    SYMBOLS.forEach(s => eng.addPrice(s, newPrices[s]));
    const newSignals = {};
    SYMBOLS.forEach(s => {
      const sig = eng.signal(s);
      newSignals[s] = sig;
      if (sig !== "HOLD") addLog(`SEMNAL ${sig} — ${s} @ $${newPrices[s]?.toFixed(2)} — nicio execuție`, sig === "BUY" ? "buy" : "sell");
    });
    setPrices(newPrices);
  ...
  useEffect(() => {
    if (running) {
      ...
      addLog("Sistema pornit — date Alpaca live", "system");
    } else {
    ...
    <div style={{ background: "#090d14", minHeight: "100vh", fontFamily: "'IBM Plex Mono','Courier New',monospace", color: "#c8d6ef", padding: 0, overflowX: "hidden" }}>
      ...
      <div style={{ background: "#0c1220", borderBottom: "1px solid #1e2d45", padding: "14px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <span style={{ fontSize: 10, color: dataSource === "live" ? "#00e5a0" : "#ffb830", marginLeft: 4 }}>● {dataSource.toUpperCase()}</span>
        </div>
      ...
      <div style={{ display: "flex", borderBottom: "1px solid #1e2d45", padding: "0 24px", marginTop: 8 }}>
        {["market", "log"].map(tab => (
      ...
      <div style={{ padding: "16px 24px 60px" }}>
        ...
        {activeTab === "log" && (
          <div>
            {log.length === 0
              ? <div style={{ color: "#4a5a72", fontSize: 12, padding: "32px 0", textAlign: "center" }}>Pornește sistemul pentru a vedea log-ul.</div>
              : log.map((entry, i) => (
